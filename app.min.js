...
const CONFIG_PADRAO = {
  ...
  kmReferencia: 300
  // distância de referência do gráfico de economia projetada (sempre km)
};
...
const T = {
  pt: {
    ...
    abastecer: "\u26FD Abastecer",
    btnGraficos: "\u{1F4CA} Gr\xE1ficos",
    grafTitulo: "\u{1F4CA} Gr\xE1ficos",
    grafVoltar: "\u2190 Voltar",
    grafSemana: "Semana",
    grafMes: "M\xEAs",
    grafAno: "Ano",
    grafVolume: () => `Volume abastecido (${uVol()})`,
    grafCusto: "Custo por per\xEDodo ($)",
    grafEconomia: "Economia por per\xEDodo ($)",
    kpiVolume: "Volume total",
    kpiCusto: "Custo total",
    kpiEconomia: "Economia l\xEDquida",
    grafSemCusto: () => usaUS() ? "Informe o pre\xE7o por gal\xE3o nos abastecimentos para ver o custo." : "Informe o pre\xE7o por litro nos abastecimentos para ver o custo.",
    grafSemEconomia: "Informe tamb\xE9m o pre\xE7o do outro combust\xEDvel para medir a economia.",
    grafConsumoLinha: () => `Consumo por combust\xEDvel (${uConsumo()})`,
    grafSemConsumoLinha: "Registre pelo menos dois abastecimentos para ver o consumo por combust\xEDvel.",
    grafCustoKm: () => `Custo por km por combust\xEDvel (${uCustoDist()})`,
    grafSemCustoKm: "Informe o pre\xE7o no abastecimento anterior para ver o custo por km deste intervalo.",
    grafEconomiaRef: (km) => `Economia projetada do etanol a cada ${fmt(distExib(km), 0)} ${uDist()}`,
    grafSemEconomiaRef: "Informe os dois pre\xE7os do posto no abastecimento para ver a economia projetada.",
    ttTotal: "total",
    ttEconomia: "economia",
    ttGasto: "gasto a mais",
    ...
    cfgKmReferencia: () => usaUS() ? "Dist\xE2ncia de refer\xEAncia (milhas)" : "Dist\xE2ncia de refer\xEAncia (km)",
    cfgFechar: "Fechar"
  },
  en: {
    ...
    abastecer: "\u26FD Fill up",
    btnGraficos: "\u{1F4CA} Charts",
    grafTitulo: "\u{1F4CA} Charts",
    grafVoltar: "\u2190 Back",
    grafSemana: "Week",
    grafMes: "Month",
    grafAno: "Year",
    ...
    grafSemCusto: () => usaUS() ? "Enter the price per gallon on fill-ups to see cost." : "Enter the price per liter on fill-ups to see cost.",
...
function agruparPorPeriodo(lista, gran, max = 12) {
  ...
  for (const r of lista) {
    ...
    const p = (_a2 = porInicio[inicio.getTime()]) != null ? _a2 : porInicio[inicio.getTime()] = vazio(inicio);
    const custo = r.precoPorLitro > 0 ? r.litros * r.precoPorLitro : 0;
    if (r.combustivel === "gasolina") {
      p.litrosGasolina += r.litros;
      p.custoGasolina += custo;
    } else {
      p.litrosEtanol += r.litros;
      p.custoEtanol += custo;
    }
    if (r.economia !== null) {
      p.economia += r.economia;
      p.temEconomia = true;
    }
  }
...
function comCustoPorKm(lista) {
  return lista.map((r, i) => {
    ...
    return __spreadProps(__spreadValues({}, r), { custoPorKm });
  });
}
function comEconomiaReferencia(lista, mediaGasolina, mediaEtanol, rendimento, kmReferencia) {
  const baseGasolina = mediaGasolina != null ? mediaGasolina : mediaEtanol ? mediaEtanol / rendimento : null;
  const baseEtanol = mediaEtanol != null ? mediaEtanol : baseGasolina ? baseGasolina * rendimento : null;
  return lista.map((r) => {
    if (!baseGasolina || !baseEtanol || !(r.precoPorLitro > 0) || !(r.precoOutro > 0)) {
      return __spreadProps(__spreadValues({}, r), { economiaReferencia: null });
    }
    ...
    return __spreadProps(__spreadValues({}, r), { economiaReferencia: custoGasolina - custoEtanol });
  });
}
const GRAF_W = 600;
const GRAF_H = 220;
...
function barraCimaPath(x, y, w, h, r) {
  ...
  return `M${px(x)},${px(y + h)} V${px(y + rr)} Q${px(x)},${px(y)} ${px(x + rr)},${px(y)} H${px(x + w - rr)} Q${px(x + w)},${px(y)} ${px(x + w)},${px(y + rr)} V${px(y + h)} Z`;
}
function barraBaixoPath(x, y, w, h, r) {
  const rr = Math.min(r, h, w / 2);
  return `M${px(x)},${px(y)} H${px(x + w)} V${px(y + h - rr)} Q${px(x + w)},${px(y + h)} ${px(x + w - rr)},${px(y + h)} H${px(x + rr)} Q${px(x)},${px(y + h)} ${px(x)},${px(y + h - rr)} Z`;
}
function gradeHtml(escala, yDe, casas) {
  return escala.ticks.map((v) => {
    const y = px(yDe(v));
    return `<line class="graf-grid" x1="${GRAF_M.esq}" y1="${y}" x2="${GRAF_W - GRAF_M.dir}" y2="${y}"/><text class="graf-eixo" x="${GRAF_M.esq - 8}" y="${y + 4}" text-anchor="end">${fmt(v, casas)}</text>`;
  }).join("");
}
function rotulosXHtml(periodos, xCentro) {
  const salto = periodos.length > 8 ? 2 : 1;
  let s = "";
  for (let i = periodos.length - 1; i >= 0; i -= salto) {
    s += `<text class="graf-eixo" x="${px(xCentro(i))}" y="${GRAF_H - 6}" text-anchor="middle">${rotuloPeriodo(periodos[i].inicio, granularidade)}</text>`;
  }
  return s;
}
...
function hitHtml(grafico, i, banda) {
  return `<rect class="graf-hit" data-grafico="${grafico}" data-i="${i}" x="${px(GRAF_M.esq + banda * i)}" y="0" width="${px(banda)}" height="${GRAF_H - GRAF_M.baixo}"/>`;
}
function svgEmpilhado(grafico, periodos, valorG, valorE, rotularUltimo) {
  ...
  periodos.forEach((p, i) => {
    ...
    if (g > 0 && e > 0) {
      marcas += `<rect class="fg-gas" x="${px(x)}" y="${px(y0 - hG)}" width="${px(geo.larg)}" height="${px(hG)}"/><path class="fg-eta" d="${barraCimaPath(x, y0 - hG - hE, geo.larg, hE, 4)}"/><line class="graf-sep" x1="${px(x)}" y1="${px(y0 - hG)}" x2="${px(x + geo.larg)}" y2="${px(y0 - hG)}"/>`;
    } else if (g > 0) {
  ...
  if (rotularUltimo && totais[iUlt] > 0) {
    rotulo = `<text class="graf-rotulo" x="${px(geo.xCentro(iUlt))}" y="${px(yDe(totais[iUlt]) - 7)}" text-anchor="middle">${fmt(totais[iUlt], 0)}</text>`;
  }
  return `<svg class="graf-svg" viewBox="0 0 ${GRAF_W} ${GRAF_H}">` + gradeHtml(escala, yDe, casas) + marcas + rotulo + rotulosXHtml(periodos, geo.xCentro) + "</svg>";
}
function svgDivergente(grafico, periodos) {
  ...
  return `<svg class="graf-svg" viewBox="0 0 ${GRAF_W} ${GRAF_H}">` + gradeHtml(escala, yDe, casas) + `<line class="graf-zero" x1="${GRAF_M.esq}" y1="${px(y0)}" x2="${GRAF_W - GRAF_M.dir}" y2="${px(y0)}"/>` + marcas + rotulosXHtml(periodos, geo.xCentro) + "</svg>";
}
...
function svgLinhas(grafico, pontos, series) {
  const todosValores = series.flatMap((s) => pontos.map(s.valor)).filter((v) => v !== null && v !== void 0);
  ...
  return `<svg class="graf-svg" viewBox="0 0 ${GRAF_W} ${GRAF_H}">` + gradeHtml(escala, yDe, casas) + (escala.fundo < 0 ? `<line class="graf-zero" x1="${GRAF_M.esq}" y1="${px(yDe(0))}" x2="${GRAF_W - GRAF_M.dir}" y2="${px(yDe(0))}"/>` : "") + marcas + rotulosXEventoHtml(pontos, geo.xCentro) + "</svg>";
}
function legendaCombustiveisHtml() {
  return `<div class="graf-legenda"><span><i class="sw sw-gas"></i>${t("gasolina")}</span><span><i class="sw sw-eta"></i>${t("etanol")}</span></div>`;
}
function cardGraficoHtml(titulo, legenda, corpo, msgVazio) {
  return `<section class="card graf-card"><h3>${titulo}</h3>` + (corpo ? legenda + corpo : `<p class="hint">${msgVazio}</p>`) + "</section>";
}
function renderGraficos() {
  ...
  const kpis = '<div class="stats">' + statHtml(t("kpiVolume"), volExib(volTotal), uVol()) + statHtml(t("kpiCusto"), custoTotal > 0 ? custoTotal : null, "$") + (econTotal !== null ? statHtml(t("kpiEconomia"), econTotal, "$", econTotal >= 0 ? "eco-pos" : "eco-neg") : "") + "</div>";
  const rendimento = (_c = (_b = (_a2 = stats.veredito) == null ? void 0 : _a2.rendimento) != null ? _b : stats.rendimentoReal) != null ? _c : config.rendimentoInicial;
  ...
  cont.innerHTML = kpis + cardGraficoHtml(
    t("grafVolume"),
    legendaCombustiveisHtml(),
    svgEmpilhado(
      "volume",
      periodosVisiveis,
      (p) => volExib(p.litrosGasolina),
      (p) => volExib(p.litrosEtanol),
      false
    ),
    t("vazio")
  ) + cardGraficoHtml(
    ...
    t("grafEconomiaRef", config.kmReferencia),
    "",
    svgLinhas("economiaReferencia", pontosLinhas.economiaReferencia, [
      {
        classeLinha: "ln-econ",
        classePonto: (v) => v >= 0 ? "pt-pos" : "pt-neg",
...
function esconderTooltipGrafico() {
  ...
  if (hitAtivo) {
    hitAtivo.classList.remove("ativo");
    hitAtivo = null;
  }
}
...
const GRAFICOS_LINHA = /* @__PURE__ */ new Set(["consumo", "custoPorKm", "economiaReferencia"]);
function mostrarTooltipGrafico(hit) {
  var _a2;
  const grafico = hit.dataset.grafico;
  const i = Number(hit.dataset.i);
  const ehLinha = GRAFICOS_LINHA.has(grafico);
  const p = ehLinha ? (_a2 = pontosLinhas[grafico]) == null ? void 0 : _a2[i] : periodosVisiveis[i];
  if (!p) return;
  ...
  if (grafico === "volume") {
    ...
    if (p.temEconomia) {
      ...
      tt.appendChild(ttLinhaEl(
        null,
        `${pos ? "+" : "\u2212"}$ ${fmt(Math.abs(p.economia), 2)}`,
        pos ? t("ttEconomia") : t("ttGasto"),
        pos ? "tt-pos" : "tt-neg"
      ));
    } else {
    ...
    if (p.economiaReferencia !== null) {
      ...
      tt.appendChild(ttLinhaEl(
        null,
        `${pos ? "+" : "\u2212"}$ ${fmt(Math.abs(p.economiaReferencia), 2)}`,
        pos ? t("ttEconomia") : t("ttGasto"),
        pos ? "tt-pos" : "tt-neg"
      ));
    } else {
  ...
  const esq = Math.max(8, Math.min(
    r.left + r.width / 2 - tt.offsetWidth / 2,
...
function aplicarTextos() {
  ...
  st("btnGraficos", "btnGraficos");
  st("grafTitulo", "grafTitulo");
  st("btnFecharGraficos", "grafVoltar");
  st("btnGranSemana", "grafSemana");
  st("btnGranMes", "grafMes");
  st("btnGranAno", "grafAno");
  ...
  st("cfgKmReferenciaLabel", "cfgKmReferencia");
...
document.getElementById("cfgKmReferencia").addEventListener("change", () => {
  ...
  if (v !== null && v >= 10 && v <= 2e3) {
    config.kmReferencia = distInterna(v);
    salvarConfig();
  }
  aplicarConfig();
});
