...
**Deploy:** the app is a PWA (`manifest.json`, `sw.js`, `icone-*.png`) published via GitHub Pages at **https://rivang81.github.io/controle-combustivel/** (repo `rivang81/controle-combustivel`, branch `main`, legacy Pages build). To ship changes: run `npm run build`, commit source + regenerated `.min.js`, `git push`, wait ~1 min. **Bump the `CACHE` version in `sw.js` on every deploy** — the service worker is cache-first, so installed clients keep old assets until the cache name changes. The SW only registers over http(s), never `file://`. Because of that cache-first behavior, a head unit that already had the site open before a deploy needs to open it **twice** (once to silently fetch/activate the new service worker in the background, once more — reload or reopen — to actually see the new assets); there's no auto-reload-on-update wired up in `app.js`.

**Installer (APK, for the tray/app drawer):** `android-twa/` is a Bubblewrap-generated Trusted Web Activity (TWA) project — a thin native Android wrapper around the deployed PWA — gitignored (contains the signing keystore, a secret, plus generated Gradle build output; never commit it to this public repo). Package id `io.github.rivang81.twa`, launcher label "Fuel". Regenerate from scratch with:
  - `npx @bubblewrap/cli init --manifest=https://rivang81.github.io/controle-combustivel/manifest.json` (interactive wizard; accept defaults except **answer "y" to "Request geolocation permission?"** — required for the speedometer's GPS — and supply throwaway signing-key details when prompted to create a new keystore).
  - `npx @bubblewrap/cli build` from inside `android-twa/`, entering the keystore/key passwords when asked. Produces `android-twa/app-release-signed.apk`.
  - Install on the head unit by copying the `.apk` via USB/SD and opening it from a file manager (enable "install unknown apps" for that file manager once).
  - **Keep `android-twa/android.keystore` and its password backed up somewhere safe outside git.** Rebuilding with a *different* keystore produces an APK Android treats as a different app — it won't update in place over a previous install; you'd have to uninstall first.
  - Known limitation: `rivang81.github.io` (the bare domain) isn't a page the user controls — it's a GitHub Pages *project* page (`.../controle-combustivel/`), so there's nowhere to host `.well-known/assetlinks.json` at the domain root for Digital Asset Link verification. The TWA therefore falls back to Chrome Custom Tabs (`fallbackType: customtabs` in `twa-manifest.json`) — the app still gets a real launcher icon and runs standalone, just with a thin URL bar at the top instead of being fully chromeless. Fixable later only by moving to a custom domain or a `rivang81.github.io` root repo the user controls.
  - The TWA renders through the head unit's installed Chrome (Custom Tabs), not a bundled WebView — it inherits whatever Chrome compatibility constraints already apply to the site (see Build above), and requires that Chrome actually be present as a system app; a head unit with only a stripped-down WebView and no real Chrome browser won't be able to launch it.
...
- **Fuel attribution for consumption (§4 note):** a record's `combustivel` is the fuel being put in the tank *now* (feeds the *next* interval). But `kmRodados`/`consumoKmL` on that same record were produced by whatever was in the tank since the *previous* fill-up — i.e. the previous record's `combustivel`. `comIntervalos()` exposes this as `combustivelConsumido` (`= registro anterior.combustivel`), and `calcularEstatisticas()`'s `porComb`/`mediaGasolina`/`mediaEtanol`/`rendimentoReal`/verdict must group by `combustivelConsumido`, never by the record's own `combustivel`. The record's own `combustivel` stays correct for anything about *this* purchase — price, cost paid, `economia`, and the volume/cost-by-period charts.
- **Per-fill-up line charts (§5.6):** the "📊 Gráficos" panel also has 3 line charts (one point per fill-up, not per calendar period) — `svgLinhas()` in `app.js`, fed by `ultimosAbastecimentos`/`comCustoPorKm`/`comEconomiaReferencia`. `comCustoPorKm` repeats the same previous-record trap as `combustivelConsumido`, but for the *price*: a record's `custoPorKm` must divide by the **previous** record's `precoPorLitro` (the price actually paid for the fuel that generated this interval), never the record's own price — and it's optional, so a missing previous price means the point is skipped (gap), not backfilled from an older price. `comEconomiaReferencia` (projected ethanol savings over `config.kmReferencia`, default 300 km) is the one exception to the previous-record rule — it uses the record's *own* `precoPorLitro`/`precoOutro` (same as `economia` in §5.4) since it's about that pump visit's prices, not about the interval's consumption; it applies the car's *current* `rendimentoReal ?? rendimentoInicial` uniformly to every point (no historical rendimento reconstruction), and — unlike the other two line charts — includes the baseline record when it has both prices.
